...
func TestRelayGatewayRoundTrip(t *testing.T) {
	...
	for _, required := range []string{"ordinary final assistant text", "Do not search for or call MCP", "automatically delivers it through Golem", "group ambient", "actor_kind identifies the current speaker type", "plain-text reference to you", "address only the current speaker's current message", "internal-only", "Never explain that no reply is needed", "[Relay completion policy]"} {
		if !strings.Contains(hint, required) {
			t.Fatalf("platform_hint %q does not contain %q", hint, required)
		}
	}
	if strings.Contains(hint, relayObserveToken) {
		t.Fatalf("platform_hint globally exposes the ambient-only observe token: %q", hint)
	}
