...
- 同一会话最多保留一个活动请求和一个合并待处理批次，避免消息积压后集中回复。
- 单轮最多发送三条文本回复；更多文本合并到第三条，图片、表情和语音独立发送。
...
PawzoChat 的角色详情页可以为 Golem bridge 配置两种回复方式：

- `模型判断`：每条消息都会交给模型。模型需要回复时正常输出；不需要回复时只输出
  `[[PAWZOCHAT_NO_REPLY]]`。PawzoChat 会把该精确标识转换为 bridge 的
  `{"outcome":"no_reply","messages":[]}`，插件收到后不会向微信发送消息。
- `按比例`：PawzoChat 先按配置比例抽样。未命中时不调用模型且不发送消息；命中时
  会明确要求模型必须回复。比例 `0` 表示始终跳过，`1` 表示每条消息都要求回复。

回复设置只影响 Golem bridge 消息。未配置回复设置的旧角色默认按比例 `1` 处理，
保持原有的每条消息都回复行为。无论是否回复，收到的消息都会先写入对应隔离角色的
对话历史，供后续上下文使用。

静默结果必须由 PawzoChat bridge 通过显式 `outcome=no_reply` 返回；普通空响应仍会被
插件视为错误，以免上游异常被误判成模型主动不回复。静默标识只在“模型判断”模式下、
且清理后的模型输出与标识完全一致时生效，不会作为普通助手消息保存。

## 队列与超时

插件为每个私聊或群聊会话维护独立队列。同一会话有请求正在等待 PawzoChat 时，后续
普通群消息会合并到一个待处理批次；私聊、`@` 机器人或引用机器人的明确消息会在该
批次中优先处理。不同会话仍可并行请求。

每次 bridge 请求都携带唯一 `request_id` 和绝对截止时间。HTTP 请求超时后，插件会
调用 PawzoChat 的取消接口。PawzoChat 会在模型调用前和回复落库、发送前检查取消或
过期状态：用户消息仍保留在角色历史中，但不会发送迟到回复。
